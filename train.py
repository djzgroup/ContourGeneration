...
from pointnet2_ops_lib.pointnet2_ops.pointnet2_modules import pooling_features
...
parser.add_argument('--dataset_path', type=str, default='./data/ABC.hdf5')
parser.add_argument('--categories', type=str_list, default=['ABC'])
...
parser.add_argument('--val_freq', type=float, default=50000)
parser.add_argument('--tag', type=str, default=None)
parser.add_argument('--num_val_batches', type=int, default=2)
parser.add_argument('--num_inspect_batches', type=int, default=2)
...
# Logging
if args.logging:
    log_dir = get_new_log_dir(args.log_root, prefix='AE_', postfix='_' +args.categories+ args.tag if args.tag is not None else '')
...
加载原始点云
'''
train_raw_dset = ShapeNetCore(
    path=args.dataset_path,
    cates=args.categories,
    split='raw_train',
    scale_mode=args.scale_mode,
)
val_raw_dset = ShapeNetCore(
    path=args.dataset_path,
    cates=args.categories,
    split='raw_test',
    scale_mode=args.scale_mode,
...
train_raw_iter = get_data_iterator(DataLoader(
    train_raw_dset,
    batch_size=args.train_batch_size,
    num_workers=0,
))
val_raw_loader = DataLoader(val_raw_dset, batch_size=args.val_batch_size, num_workers=0)
...
def train(it):
    ...
    raw_batch = next(train_raw_iter)
    raw_x = raw_batch['pointcloud'].to(args.device)
...
def validate_loss(it):
    ...
    all_refs = []
    all_recons = []
    for i, batch in enumerate(tqdm(val_raw_loader, desc='Validate')):
...
def validate_inspect(it):
    sum_n = 0
    sum_chamfer = 0
    for i, batch in enumerate(tqdm(val_raw_loader, desc='Inspect')):
        x = batch['pointcloud'].to(args.device)
        model.eval()
        code = model.encode(x)
