...
from models.encoders.pointnet_util import index_points, square_distance
...
class FPS_kNN(nn.Module):
    ...
    def forward(self, xyz, x):
        ...
        B, N, _ = xyz.shape# batch size ,num points
...
class Point_NN(nn.Module):
    def __init__(self,args, input_points=2048, num_stages=4, embed_dim=72, k_neighbors=90, beta=1000, alpha=100):
        super().__init__()
        # Non-Parametric Encoder
        self.EncNP = EncNP(input_points, num_stages, embed_dim, k_neighbors, alpha, beta)
        self.dgcnn=DGCNN_cls(args)
        # Mapping to [c], cmean

        self.fc1_m = nn.Linear(1152, 512)
        self.fc2_m = nn.Linear(512, 256)
        self.fc3_m = nn.Linear(256,512)

        self.fc_bn1_m = nn.BatchNorm1d(512)
        self.fc_bn2_m = nn.BatchNorm1d(256)

        # Mapping to [c], cmean
        self.fc1_v = nn.Linear(1152, 512)
        self.fc2_v = nn.Linear(512, 256)
        self.fc3_v = nn.Linear(256, 512)
